...
def save_audio_file(buf_memory_containing_audio, filename_prefix, output_dir, logger=None):
    """
    ...
    try:
        ...
        if logger:
            logger.error(f"Failed to save audio file {filename_prefix}.wav: {e}")
        else:
            print(f"❌ Failed to save audio file {filename_prefix}.wav: {e}")
        raise e

def save_spectrogram_image(buf_memory_containing_audio, filename_prefix, output_dir, logger=None):
    """
    ...
    try:
        ...
        plt.close(fig)
        if logger:
            logger.debug(f"Spectrogram saved at: {output_path}")
        else:
            print(f"Spectrogram saved at: {output_path}")
    except Exception as e:
        if logger:
            logger.error(f"Failed to save spectrogram image {filename_prefix}.jpg: {e}")
        else:
            print(f"❌ Failed to save spectrogram image {filename_prefix}.jpg: {e}")
        raise e
